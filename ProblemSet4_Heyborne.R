...
Diphtheria (coef 6.72), Adult.Mortality (coef -2.29), Total.expenditure (coef 1.99), GDP (coef 7.27), and Schooling (coef 1.18). 

HIV.AIDS and Adult.mortality predicted negative effects on life expectancy, while Diphtheria, Total.expenditure, GDP, and School predicted positive effects 
on life expectancy. I choose these predictors because they were the best predictors from the grouped linear models (illness, mortality, and expenditure)
that I checked first. Additionally, Diptheria and Schooling were the better preforming predictors relative to the predictors they had high collinearity
with (Polio and Income.composition) (since I had to remove one of the predictors from each high collinearity pair, they were the obvious choice). Aside from 
those 2 high collinearity varaibles I did not include percentage.expenditure (because it was not significant in the model), HepB/Measles/under.five.deaths 
(because their effect sizes were small), and infant.deaths (because its effect was strangely positive). Lastly, I did not include 
either lifestyle predictor, alcohol and BMI, because their effect was also strangely positive. I figured that this was likely the effect that the outcome 
(life expectancy) had on the predictors (alcohol and BMI) because people tend to gain weight and drink more as they age. This left me with my "best model" 
...
up 3/6 of the the best model predictors in the end. My inturpretation of the domination of economic predictors in the best model is that life expectency
is heavily influenced by variables that relate to economic status (schooling isnt strictly an economic status variable, but I would suspect that theres 
overlap between economic status and years of schooling). HIV.AIDS likely had a strong negative effect due to the high mortality rate associated with the 
condition. Dipthermia was the best predictor amoungst the vaccine group, and supports the notion that vaccinations improve life expectancy. Finally, adult 
mortality has a strong effect on life expectancy but this really doesnt tell us much, since we would assume that adult mortality would negativly effect life 
expectancy like we see in the model. It boosts model preformance but is not very meaningful for making life expectancy inferences. Overall, the "best" linear
model (with 6 predictors) explains 78% of the variablity of life expectany outcomes with a residual standard error of 4.4 years.
'''
